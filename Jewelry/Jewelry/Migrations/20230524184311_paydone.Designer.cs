// <auto-generated />
using System;
using Jewelry.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Jewelry.Migrations
{
    [DbContext(typeof(JewelryContext))]
    [Migration("20230524184311_paydone")]
    partial class paydone
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Jewelry.Models.Addtoshop", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("pName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("price")
                        .HasColumnType("int");

                    b.Property<int>("quantity")
                        .HasColumnType("int");

                    b.Property<int>("totalprice")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Addtoshops");
                });

            modelBuilder.Entity("Jewelry.Models.BrandMst", b =>
                {
                    b.Property<int>("BrandMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("BrandMstID"));

                    b.Property<string>("Brand_Type")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("BrandMstID");

                    b.ToTable("brandMsts");
                });

            modelBuilder.Entity("Jewelry.Models.CartItem", b =>
                {
                    b.Property<int>("ProductId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ProductId"));

                    b.Property<string>("Image")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("Price")
                        .HasColumnType("float");

                    b.Property<string>("ProductName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.HasKey("ProductId");

                    b.ToTable("cartItems");
                });

            modelBuilder.Entity("Jewelry.Models.CatMst", b =>
                {
                    b.Property<int>("CatMstId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("CatMstId"));

                    b.Property<string>("Cat_Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("CatMstId");

                    b.ToTable("catMsts");
                });

            modelBuilder.Entity("Jewelry.Models.CertifyMst", b =>
                {
                    b.Property<int>("CertifyMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("CertifyMstID"));

                    b.Property<string>("Cat_Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("CertifyMstID");

                    b.ToTable("CertifyMsts");
                });

            modelBuilder.Entity("Jewelry.Models.DimMst", b =>
                {
                    b.Property<int>("DimMstId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("DimMstId"));

                    b.Property<int>("DimQltyMstID")
                        .HasColumnType("int");

                    b.Property<int>("DimQltySubMstID")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Amt")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Crt")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Gm")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Pcs")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Rate")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Size")
                        .HasColumnType("int");

                    b.Property<int>("ItemMstID")
                        .HasColumnType("int");

                    b.HasKey("DimMstId");

                    b.HasIndex("DimQltyMstID");

                    b.HasIndex("DimQltySubMstID");

                    b.HasIndex("ItemMstID");

                    b.ToTable("dimMsts");
                });

            modelBuilder.Entity("Jewelry.Models.DimQltyMst", b =>
                {
                    b.Property<int>("DimQltyMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("DimQltyMstID"));

                    b.Property<string>("DimQlty")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("DimQltyMstID");

                    b.ToTable("dimQltyMsts");
                });

            modelBuilder.Entity("Jewelry.Models.DimQltySubMst", b =>
                {
                    b.Property<int>("DimQltySubMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("DimQltySubMstID"));

                    b.Property<string>("DimQlty")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("DimQltySubMstID");

                    b.ToTable("dimQltySubMsts");
                });

            modelBuilder.Entity("Jewelry.Models.GoldKrtMst", b =>
                {
                    b.Property<int>("GoldKrtMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("GoldKrtMstID"));

                    b.Property<string>("Gold_Crt")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("GoldKrtMstID");

                    b.ToTable("goldKrtMsts");
                });

            modelBuilder.Entity("Jewelry.Models.ItemDetails", b =>
                {
                    b.Property<int>("ItemMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ItemMstID"));

                    b.Property<int>("BrandMstID")
                        .HasColumnType("int");

                    b.Property<int>("CatMstId")
                        .HasColumnType("int");

                    b.Property<int>("CertifyMstID")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Amt")
                        .HasColumnType("int");

                    b.Property<int>("Dim_Pcs")
                        .HasColumnType("int");

                    b.Property<int>("GoldKrtMstID")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Amt")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Making")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Rate")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Wt")
                        .HasColumnType("int");

                    b.Property<int>("MRP")
                        .HasColumnType("int");

                    b.Property<int>("Net_Gold")
                        .HasColumnType("int");

                    b.Property<int>("Pairs")
                        .HasColumnType("int");

                    b.Property<int>("ProdMstID")
                        .HasColumnType("int");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Making")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Wt")
                        .HasColumnType("int");

                    b.Property<int>("Tot_Gross_Wt")
                        .HasColumnType("int");

                    b.Property<int>("Wstg")
                        .HasColumnType("int");

                    b.Property<int>("Wstg_Per")
                        .HasColumnType("int");

                    b.Property<string>("imgUrl")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("prodName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ItemMstID");

                    b.HasIndex("BrandMstID");

                    b.HasIndex("CatMstId");

                    b.HasIndex("CertifyMstID");

                    b.HasIndex("GoldKrtMstID");

                    b.HasIndex("ProdMstID");

                    b.ToTable("itemDetails");
                });

            modelBuilder.Entity("Jewelry.Models.ItemMst", b =>
                {
                    b.Property<int>("ItemMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ItemMstID"));

                    b.Property<int>("BrandMstID")
                        .HasColumnType("int");

                    b.Property<int>("CatMstId")
                        .HasColumnType("int");

                    b.Property<int>("CertifyMstID")
                        .HasColumnType("int");

                    b.Property<int>("GoldKrtMstID")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Amt")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Making")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Rate")
                        .HasColumnType("int");

                    b.Property<int>("Gold_Wt")
                        .HasColumnType("int");

                    b.Property<int>("MRP")
                        .HasColumnType("int");

                    b.Property<int>("Net_Gold")
                        .HasColumnType("int");

                    b.Property<int>("Other_Making")
                        .HasColumnType("int");

                    b.Property<int>("Pairs")
                        .HasColumnType("int");

                    b.Property<int>("ProdMstID")
                        .HasColumnType("int");

                    b.Property<string>("Prod_Quality")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Making")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Wt")
                        .HasColumnType("int");

                    b.Property<int>("Tot_Gross_Wt")
                        .HasColumnType("int");

                    b.Property<int>("Tot_Making")
                        .HasColumnType("int");

                    b.Property<int>("Wstg")
                        .HasColumnType("int");

                    b.Property<int>("Wstg_Per")
                        .HasColumnType("int");

                    b.HasKey("ItemMstID");

                    b.HasIndex("BrandMstID");

                    b.HasIndex("CatMstId");

                    b.HasIndex("CertifyMstID");

                    b.HasIndex("GoldKrtMstID");

                    b.HasIndex("ProdMstID");

                    b.ToTable("itemMsts");
                });

            modelBuilder.Entity("Jewelry.Models.Payment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CVV")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("Cardholdername")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CreditCard")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("nvarchar(16)");

                    b.Property<DateTime>("DateTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TotalAmount")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("payments");
                });

            modelBuilder.Entity("Jewelry.Models.ProdMst", b =>
                {
                    b.Property<int>("ProdMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ProdMstID"));

                    b.Property<string>("Prod_Type")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("ProdMstID");

                    b.ToTable("prodMsts");
                });

            modelBuilder.Entity("Jewelry.Models.StoneMst", b =>
                {
                    b.Property<int>("StoneMstId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("StoneMstId"));

                    b.Property<int>("ItemMstID")
                        .HasColumnType("int");

                    b.Property<int>("StoneQltyMstID")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Amt")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Crt")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Gm")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Pcs")
                        .HasColumnType("int");

                    b.Property<int>("Stone_Rate")
                        .HasColumnType("int");

                    b.HasKey("StoneMstId");

                    b.HasIndex("ItemMstID");

                    b.HasIndex("StoneQltyMstID");

                    b.ToTable("stoneMsts");
                });

            modelBuilder.Entity("Jewelry.Models.StoneQltyMst", b =>
                {
                    b.Property<int>("StoneQltyMstID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("StoneQltyMstID"));

                    b.Property<string>("StoneQlty")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("StoneQltyMstID");

                    b.ToTable("stoneQltyMsts");
                });

            modelBuilder.Entity("Jewelry.Models.UserRegMst", b =>
                {
                    b.Property<int>("UserRegMstId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("UserRegMstId"));

                    b.Property<string>("address")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("cdate")
                        .HasColumnType("datetime2");

                    b.Property<string>("city")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime>("dob")
                        .HasColumnType("datetime2");

                    b.Property<string>("emailID")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)");

                    b.Property<string>("mobNo")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("password")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("state")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("userFname")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("userLname")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.HasKey("UserRegMstId");

                    b.ToTable("userRegMsts");
                });

            modelBuilder.Entity("Jewelry.Models.pay", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CVV")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("Cardholdername")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CreditCard")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("nvarchar(16)");

                    b.Property<DateTime>("DateTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("TotalAmount")
                        .HasColumnType("float");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("pays");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("ProviderKey")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("Name")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("Jewelry.Models.DimMst", b =>
                {
                    b.HasOne("Jewelry.Models.DimQltyMst", "dimQltyMst")
                        .WithMany()
                        .HasForeignKey("DimQltyMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.DimQltySubMst", "dimQltySubMst")
                        .WithMany()
                        .HasForeignKey("DimQltySubMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.ItemMst", "itemMst")
                        .WithMany()
                        .HasForeignKey("ItemMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("dimQltyMst");

                    b.Navigation("dimQltySubMst");

                    b.Navigation("itemMst");
                });

            modelBuilder.Entity("Jewelry.Models.ItemDetails", b =>
                {
                    b.HasOne("Jewelry.Models.BrandMst", "brandMst")
                        .WithMany()
                        .HasForeignKey("BrandMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.CatMst", "catMst")
                        .WithMany()
                        .HasForeignKey("CatMstId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.CertifyMst", "certifyMst")
                        .WithMany()
                        .HasForeignKey("CertifyMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.GoldKrtMst", "goldKrtMst")
                        .WithMany()
                        .HasForeignKey("GoldKrtMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.ProdMst", "prodMst")
                        .WithMany()
                        .HasForeignKey("ProdMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("brandMst");

                    b.Navigation("catMst");

                    b.Navigation("certifyMst");

                    b.Navigation("goldKrtMst");

                    b.Navigation("prodMst");
                });

            modelBuilder.Entity("Jewelry.Models.ItemMst", b =>
                {
                    b.HasOne("Jewelry.Models.BrandMst", "brandMst")
                        .WithMany()
                        .HasForeignKey("BrandMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.CatMst", "catMst")
                        .WithMany()
                        .HasForeignKey("CatMstId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.CertifyMst", "certifyMst")
                        .WithMany()
                        .HasForeignKey("CertifyMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.GoldKrtMst", "goldKrtMst")
                        .WithMany()
                        .HasForeignKey("GoldKrtMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.ProdMst", "prodMst")
                        .WithMany()
                        .HasForeignKey("ProdMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("brandMst");

                    b.Navigation("catMst");

                    b.Navigation("certifyMst");

                    b.Navigation("goldKrtMst");

                    b.Navigation("prodMst");
                });

            modelBuilder.Entity("Jewelry.Models.StoneMst", b =>
                {
                    b.HasOne("Jewelry.Models.ItemMst", "ItemMst")
                        .WithMany()
                        .HasForeignKey("ItemMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Jewelry.Models.StoneQltyMst", "ItemQltyMst")
                        .WithMany()
                        .HasForeignKey("StoneQltyMstID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ItemMst");

                    b.Navigation("ItemQltyMst");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
